...
def inception_resnet_A_block(net,name):
    with tf.name_scope(name):
        ...
        with tf.name_scope(name+'/Merge_Conv_1x1'):
            conv = conv2d(conv,n_filters=384,
                              k_h=1,k_w=1,
                              stride_h=1,stride_w=1,
                              padding='SAME',
                              activation=None,
...
def reduction_A_block(net,name):
    with tf.name_scope(name):
        ...
        with tf.name_scope(name+'/Branch_1'):
            ...
            print(name+'/Branch_1,shape={0}'.format(branch_1.get_shape()))
        with tf.name_scope(name+'/Branch_2'):
            branch_2 = conv2d(net,n_filters=256,#k
                                  k_h=1,k_w=1,
                                  stride_h=1,stride_w=1,
                                  padding='SAME',
                                  name=name+'/Branch_2/Conv2d_2a_1x1')
            branch_2 = conv2d(branch_2,n_filters=256,#l
                                  k_h=3,k_w=3,
                                  stride_h=1,stride_w=1,
                                  padding='SAME',
                                  name=name+'/Branch_2/Conv2d_2b_3x3')
            branch_2 = conv2d(branch_2,n_filters=384,#m
                                  k_h=3,k_w=3,
                                  stride_h=2,stride_w=2,
                                  padding='VALID',
                                  name=name+'/Branch_2/Conv2d_2c_3x3')
...
def inception_resnet_B_block(net,name):
    with tf.name_scope(name):
        ...
        with tf.name_scope(name+'/Merge_Conv_1x1'):
            conv = conv2d(conv,n_filters=1152,
                              k_h=1,k_w=1,
                              stride_h=1,stride_w=1,
                              padding='SAME',
                              activation=None,
    ...
    with tf.name_scope(name):
        ...
        with tf.name_scope(name+'/Branch_2'):
            branch_2 = conv2d(net,n_filters=256,
                                  ...
                                  padding='SAME',
                                  name=name+'/Branch_2/Conv2d_2a_1x1')
            branch_2 = conv2d(branch_2,n_filters=288,
                                  k_h=3,k_w=3,
                                  stride_h=2,stride_w=2,
                                  padding='VALID',
                                  name=name+'/Branch_2/Conv2d_2b_3x3')
        ...
        with tf.name_scope(name+'/Branch_3'):
            ...
            branch_3 = conv2d(branch_3,n_filters=288,
                                  k_h=3,k_w=3,
                                  stride_h=1,stride_w=1,
                                  padding='SAME',
                                  name=name+'/Branch_3/Conv2d_3b_3x3')
            branch_3 = conv2d(branch_3,n_filters=320,
                                  k_h=3,k_w=3,
                                  stride_h=2,stride_w=2,
                                  padding='VALID',
...
def inception_resnet_C_block(net,name):
    with tf.name_scope(name):
        ...
        with tf.name_scope(name+'/Merge_Conv_1x1'):
            conv = conv2d(conv,n_filters=2048,
                              k_h=1,k_w=1,
                              stride_h=1,stride_w=1,
                              padding='SAME',
                              activation=None,
...
def train_inception_resnet_v2_stacked_lstm(width,
    ...
    y_inception = inception_resnet_v2(x,width,height,keep_prob)
...
if __name__ == '__main__':
    if len(sys.argv) == 3:
        w = int(sys.argv[1])
        h = int(sys.argv[2])
        print('...... training inception_resnet_v2 and stacked lstm network:width = {0},height = {1}'.format(sys.argv[1],sys.argv[2]))
        train_inception_resnet_v2_stacked_lstm(width=w,height=h)

    if len(sys.argv) == 4:
        w = int(sys.argv[1])
        h = int(sys.argv[2])
        print('...... testing inception_resnet_v2 and stacked lstm network:width = {0},height = {1}'.format(sys.argv[1],sys.argv[2]))
